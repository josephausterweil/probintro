...
The same algorithm that ranks web pages also models **human memory**. Griffiths, Steyvers and Firl (2007), in a paper titled *Google and the Mind*, ran PageRank over a **semantic** network (built from word-association data) and found its stationary distribution predicts how readily people produce each word — high-$\pi$ concepts are the ones that "come to mind" first. Same walk, same $\pi$, a $100-billion idea on one side and a theory of recall on the other. That second story is [Chapter 15](../15_memory_search/).
{{% /notice %}}
